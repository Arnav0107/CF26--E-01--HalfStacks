...
export default function UpdateClaimForm({ claims, onSuccessSubmit, API_URL }) {
  ...
  return (
    <div className="glass rounded-xl border border-white/5 shadow-xl p-6 h-full flex flex-col justify-between min-h-[500px]">
      <div>
        ...
        <div className="flex items-center justify-between border-b border-white/5 pb-4 mb-4">
          ...
          <div className="flex items-center gap-1.5 text-3xs font-mono text-slate-500">
            <span className={step === 1 ? "text-brand-400 font-bold" : ""}>1. Select Target</span>
            <span>&bull;</span>
            <span className={step === 2 ? "text-brand-400 font-bold" : ""}>2. Diff Review</span>
            <span>&bull;</span>
            <span className={step === 3 ? "text-brand-400 font-bold" : ""}>3. Verification Result</span>
          </div>
        </div>
          ...
          <div className="space-y-4">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-dark-900/50 border border-white/10 rounded-lg pl-9 pr-4 py-2 text-xs text-white placeholder-slate-600 focus:outline-none focus:border-brand-500"
              />
            </div>

            <div className="border border-white/5 rounded-lg overflow-hidden max-h-[220px] overflow-y-auto bg-dark-950/20 divide-y divide-white/5">
                  ...
                  return (
                    <div
                      ...
                      className={`p-3 text-xs flex justify-between items-center cursor-pointer transition-all ${
                        isChosen ? 'bg-brand-500/10 border-l-2 border-brand-500 text-white' : 'text-slate-400 hover:bg-white/5'
                      }`}
                    >
                      <div className="space-y-1">
                        <span className="font-mono text-brand-400 font-bold">{c.claimId}</span>
                        <div className="font-bold text-slate-200">{c.projectName}</div>
                        <div className="text-slate-500 text-3xs font-mono">{c.orgId.substring(0, 16)}...</div>
                      </div>
                      <div className="text-right">
                        <div className="font-mono text-slate-200 font-semibold">{Number(c.tonnage).toLocaleString()} t</div>
                        <span className="bg-dark-800 px-2 py-0.5 rounded text-[10px] text-slate-400 font-mono">v{c.version}</span>
                      </div>
                    </div>
                  );
                })
            ...
            <div className="flex justify-end pt-2">
              <button
                ...
                className="bg-brand-500 hover:bg-brand-600 text-white rounded-lg px-4 py-2 text-xs font-bold transition-all disabled:opacity-50 flex items-center gap-1.5"
              >
                Next: Diff Review
        ...
        {step === 2 && selectedClaim && (
          <form onSubmit={handleSubmitCorrection} className="space-y-4">
            {submitError && (
              <div className="p-3 rounded-lg bg-red-500/10 border border-red-500/25 text-red-400 text-2xs flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 gap-4 text-xs bg-white/5 p-3 rounded-lg border border-white/5 font-mono">
              <div>
                <span className="text-slate-500 text-3xs uppercase block">Project Name:</span>
                <span className="text-white font-semibold truncate block">{selectedClaim.projectName}</span>
              </div>
              <div>
                <span className="text-slate-500 text-3xs uppercase block">Methodology:</span>
                <span className="text-white font-semibold truncate block">{selectedClaim.projectType}</span>
              </div>
              <div className="col-span-2 border-t border-white/5 pt-2 mt-2">
                ...
                <span className="text-slate-300 text-3xs flex items-center gap-1">
                  <Key className="h-3 w-3 text-brand-500 shrink-0" />
                  {selectedClaim.orgId}
                </span>
              </div>
            </div>
            ...
            <div>
              <label className="block text-2xs font-bold text-slate-400 uppercase tracking-wider mb-2 text-center">
                Tonnage Difference Comparison
              </label>
              <div className="grid grid-cols-2 gap-4">
                {/* BEFORE */}
                <div className="p-4 bg-red-500/5 border border-red-500/10 rounded-lg text-center space-y-1">
                  <span className="text-red-400 font-bold uppercase tracking-wider text-[10px]">Before (v{selectedClaim.version})</span>
                ...
                <div className="p-4 bg-emerald-500/5 border border-emerald-500/10 rounded-lg text-center space-y-1">
                  <span className="text-emerald-400 font-bold uppercase tracking-wider text-[10px]">After (v{selectedClaim.version + 1})</span>
                  <input
                    type="number"
                    value={newTonnage}
                    ...
                    className="w-full bg-dark-900/50 border border-white/10 rounded-lg px-2 py-1 text-sm text-center text-white placeholder-slate-600 focus:outline-none focus:border-brand-500 font-mono font-bold"
                  />
            ...
            <div className="flex justify-between items-center pt-2">
              <button
                ...
                className="border border-white/10 hover:border-white/20 text-slate-400 rounded-lg px-4 py-2 text-xs font-bold transition-all flex items-center gap-1.5"
              >
              ...
              <button
                type="submit"
                disabled={submitting}
                className="bg-red-600 hover:bg-red-500 text-white rounded-lg px-4 py-2 text-xs font-bold transition-all disabled:opacity-50 flex items-center gap-1.5 shadow-lg"
              >
                {submitting ? (
                  <>
          ...
          <div className="space-y-4">
            {verifying ? (
              <div className="p-8 text-center space-y-3">
                ...
                <p className="text-xs text-slate-400 font-mono">Running cryptographic verification chain checks...</p>
              </div>
            ) : verifyReport ? (
              <div className="space-y-4">
                ...
                <div className="space-y-2 text-2xs font-mono">
                  {/* Check 1 */}
                  <div className="flex justify-between items-center p-2 border border-white/5 bg-dark-900/40 rounded">
                    <span className="text-slate-400">1. Payload Hash Recomputed:</span>
                    <span className={`px-2 py-0.5 rounded font-bold ${
                      getHashRecomputeStatus() ? 'bg-emerald-500/10 text-emerald-400' : 'bg-red-500/10 text-red-400'
                    }`}>
                  ...
                  {/* Check 2 */}
                  <div className="flex justify-between items-center p-2 border border-white/5 bg-dark-900/40 rounded">
                    <span className="text-slate-400">2. ECDSA Signature Re-checked:</span>
                    <span className={`px-2 py-0.5 rounded font-bold ${
                  ...
                  {/* Check 3 */}
                  <div className="flex justify-between items-center p-2 border border-white/5 bg-dark-900/40 rounded">
                    <span className="text-slate-400">3. On-Chain Anchor Re-derived:</span>
                    <span className={`px-2 py-0.5 rounded font-bold ${
                  ...
                  {/* Check 4 */}
                  <div className="flex justify-between items-center p-2 border border-white/5 bg-dark-900/40 rounded">
                    <span className="text-slate-400">4. Parent History Chain Walked:</span>
                    <span className={`px-2 py-0.5 rounded font-bold ${
                ...
                {verifyReport.errors && verifyReport.errors.length > 0 && (
                  <div className="p-3 bg-red-500/10 border border-red-500/25 rounded-lg text-3xs font-mono text-red-300">
                    <span className="font-bold block mb-1 uppercase text-red-400">Diagnostics:</span>
                ...
                <button
                  ...
                  className="w-full bg-brand-500 hover:bg-brand-600 text-white rounded-lg py-2.5 font-bold transition-all text-xs flex items-center justify-center gap-1.5"
                >
